...
const SEGMENTS = [
  {
    key: "高价值用户",
    name: "高价值用户",
    description: "价值评分处于前 25%，交易贡献和综合活跃度最高。",
  },
  {
    key: "潜力用户",
    name: "潜力用户",
    description: "价值评分处于 50%–75% 区间，已有较好行为基础并具备成长空间。",
  },
  {
    key: "普通用户",
    name: "普通用户",
    description: "价值评分处于 25%–50% 区间，当前贡献和活跃度处于中低水平。",
  },
  {
    key: "低价值用户",
    name: "低价值用户",
    description: "价值评分处于后 25%，当前交易贡献和活跃程度较低。",
  },
];
...
function assignValueSegments(users) {
  const ranked = users
    .map((user, originalIndex) => ({
      ...user,
      originalIndex,
      scoreForRank: toNumber(user.value_score),
    }))
  ...
  return ranked
    .map((user, rankIndex) => {
      const percentile = rankIndex / total;
      let valueLevel = "低价值用户";

      if (percentile < 0.25) valueLevel = "高价值用户";
      else if (percentile < 0.5) valueLevel = "潜力用户";
      else if (percentile < 0.75) valueLevel = "普通用户";

      return {
        ...user,
        value_level_display: valueLevel,
      };
    })
...
function buildProfile(segmentUsers, totalUsers) {
  ...
  return {
    ...
    avg_value_score: Number(
      (
        segmentUsers.reduce((sum, user) => sum + toNumber(user.value_score), 0) /
        divisor
      ).toFixed(2)
    ),
...
function ValueAnalysis() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      console.error("读取用户价值分析数据失败", error);
    }
  }, []);

  const valueUsers = useMemo(() => assignValueSegments(users), [users]);

  const segmentData = useMemo(
    () =>
      SEGMENTS.map((segment) => {
        const segmentUsers = valueUsers.filter(
          (user) => user.value_level_display === segment.key
        );

        return {
          ...segment,
          users: segmentUsers,
          profile: buildProfile(segmentUsers, valueUsers.length),
        };
      }),
    [valueUsers]
  );
  ...
  async function generateAIStrategy(segment) {
    ...
    try {
      const strategy = await generateStrategy("value", {
      ...
      console.error("AI价值策略生成失败", error);
      setAiStrategies((previous) => ({
        ...previous,
  ...
  return (
    <div className="page page-segmentation analysis-module-page">
      <div className="glass-panel info-card analysis-module-header">
        <div className="page-title">用户价值分析</div>
        <p>基于价值评分、交易贡献和链上行为对用户进行价值分层。</p>
      </div>
        ...
        {segmentData.map((segment) => (
          <div className="glass-card analysis-segment-detail-card" key={segment.key}>
            ...
            <div className="analysis-profile-metrics">
              <div>
                <span>平均价值评分</span>
                <strong>{segment.profile.avg_value_score}</strong>
              </div>
              <div>
                <span>平均交易价值</span>
            ...
            <div className="analysis-ai-panel">
              ...
              {aiStrategies[segment.key] ? (
                ...
                >
                  {loadingLevel === segment.key ? "AI分析中..." : "生成AI价值策略"}
                </button>
              )}
            </div>
          </div>
      ...
      <div className="section-title">自定义用户分层</div>
      <div className="glass-card analysis-custom-segment-card">
        <p>如果默认价值分层不能满足业务需求，可以继续创建自定义用户群体。</p>
        <button className="primary-button" onClick={() => navigate("/segment-builder")}>
          创建自定义用户分层
        </button>
      </div>
    </div>
